<template>
  <div class="min-h-screen flex flex-col md:flex-row overflow-hidden">
    <!-- 🎛️ KIRI: FORM INPUT -->
    <div
      class="md:w-1/2 w-full bg-white flex flex-col p-10 border-b md:border-b-0 md:border-r border-gray-200"
    >
      <!-- Header -->
      ...
      <div class="space-y-5 flex-grow overflow-auto p-2">
        ...
        <div class="mt-6">
          ...
          >
            ...
            />
            <select
              v-model="col.sortColumn"
              class="border p-2 rounded bg-white flex-1"
            >
            ...
            <input
              v-model="col.class"
              placeholder="class (contoh: text-center)"
              class="border p-2 rounded flex-1"
            />
            ...
            <button
              ...
              class="text-red-500 hover:text-red-700 transition flex items-center justify-center ml-auto px-2"
              title="Hapus kolom"
            >
              ✖
            </button>
...
const columns = ref([
  ...
  { text: "Nama", sortBy: "name", sortColumn: true, class: "w-200px" },
...
function addColumn() {
  columns.value.push({ text: "", sortBy: "", sortColumn: false, class: "" });
}
